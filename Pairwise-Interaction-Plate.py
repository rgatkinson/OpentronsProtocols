...
trash_control = False  # False will return trip to rack (use for debugging only)
...
def hasMoreTips(p, tips_needed: int) -> bool:
    ...
    try:
        # Try to pull 'count' tips. An exception will throw if there's not enough
        nxt = next(p.tip_rack_iter)
        for i in range(1, tips_needed):
...
for iVolume in range(0, len(strand_volumes)):
    ...
    p.transfer(strand_volumes[iVolume], strand_a, strandAWells(iVolume), new_tip='always', trash=trash_control)
...
for index in range(0, len(strand_volumes)):
    ...
    p.transfer(strand_volumes[index], strand_b, strandBWells(index), new_tip='always', trash=trash_control, mix_after=(4, mix_vol))
